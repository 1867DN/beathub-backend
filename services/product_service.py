...
class ProductService(BaseServiceImpl):
    ...
    def get_all(self, skip: int = 0, limit: int = 100, include_inactive: bool = False) -> List[ProductSchema]:
        """
        Get all products with caching. By default only returns active products.

        Cache key pattern: products:list:skip:{skip}:limit:{limit}:inactive:{include_inactive}
        ...
        # Cache miss - get from database with is_active filter
        logger.debug(f"Cache MISS: {cache_key}")
        query = self._repository.session.query(ProductModel)
        if not include_inactive:
            query = query.filter(ProductModel.is_active != 'false')
        products_raw = query.offset(skip).limit(limit).all()
        products = [ProductSchema.model_validate(p) for p in products_raw]
    ...
    def get_by_brand(self, brand_id: int, skip: int = 0, limit: int = 100) -> List[ProductSchema]:
        """Get all active products that belong to a specific brand."""
        products = self._repository.session.query(ProductModel).filter(
