...
class BaseRepositoryImpl(BaseRepository):
    ...
    def _model_to_dict(self, model: BaseModel) -> dict:
        """
        Convert a SQLAlchemy model instance to a plain dict containing only
        column attributes (no relationships) to avoid recursive structures
        when validating with Pydantic.
        """
        result = {}
        try:
    ...
    def find(self, id_key: int) -> BaseSchema:
        ...
        try:
            ...
            return self.schema.model_validate(self._model_to_dict(model))
    ...
    def find_all(self, skip: int = 0, limit: int = 100) -> List[BaseSchema]:
        ...
        try:
            ...
            return [self.schema.model_validate(self._model_to_dict(model)) for model in models]
    ...
    def update(self, id_key: int, changes: dict) -> BaseSchema:
        ...
        try:
            ...
            return self.schema.model_validate(self._model_to_dict(instance))
    ...
    # Note: _model_to_dict implemented earlier (columns-only conversion)
    ...
    def save_all(self, models: List[BaseModel]) -> List[BaseSchema]:
        ...
        try:
            ...
            # Refresh all models and return plain dicts to avoid ORM recursion
            for model in models:
                self.session.refresh(model)
